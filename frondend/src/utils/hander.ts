...
type TCallback<T = Partial<TaroGeneral.CallbackResult>> = (
  res: T
) => Promise<void> | void;
...
export class MethodHandler<T = Partial<TaroGeneral.CallbackResult>> {
  ...
  constructor({ name, success, fail, complete }: IMethodParam<T>) {
    ...
    this.isHandlerError =
      isFunction(this.__complete) || isFunction(this.__fail);
  }
...
type TCallbackManagerFunc<T extends unknown[] = unknown[]> = (
  ...arr: T
) => void;
...
type TCallbackManagerUnit<T extends unknown[] = unknown[]> =
  | TCallbackManagerFunc<T>
  | ICallbackManagerOption<T>;
...
export function shouldBeObject(target: unknown) {
  ...
  return {
    flag: false,
    msg: `parameter should be an Object, but got ${typeof target}`,
  };
}
