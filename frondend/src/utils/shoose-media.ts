...
export const chooseMedia = async function (
  ...
  if (count && typeof count !== "number") {
    res.errMsg = `${methodName}:fail parameter error: count must be a number`;
    ...
    if (sourceType.length > 1 || sourceType.length < 1) {
      try {
        const { tapIndex } = await showActionSheet(
          {
            itemList: [
              options.cameraText ? options.cameraText : "拍摄",
              options.albumText ? options.albumText : "从相册选择",
            ],
  ...
  return new Promise<Taro.chooseMedia.SuccessCallbackResult>(
    (resolve, reject) => {
      if (!el) return;
      ...
      el.onchange = async function (e) {
        ...
        handle.success(res, {
          ...
          reject: reject as unknown as any,
        });
        target.value = "";
      };
      el.onabort = () =>
